...
运行时对白、目标、提示、章节标题、地图标签、实体名称、物件、日记、记忆页、科普页、错误提示和无障碍文本均使用稳定 ID / `TextRef`，由 `src/i18n/catalogs.ts` 的三套 catalog 渲染。Tiled 的中文 `label` 只保留给编辑器；运行时按实体稳定 ID 查找翻译。语言变化只更新 DOM 和 Canvas 实体文字，不重建章节、不改变玩法或进度。
