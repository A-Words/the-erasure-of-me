...
> 版本：v0.1
> 状态：垂直切片可录入稿
> 内容基线：简体中文；运行时支持 zh-CN、zh-HK、en，默认跟随浏览器系统语言，设置页可手动覆盖
...
运行时对白、目标、提示、章节标题、地图标签、实体名称、物件、日记、记忆页、科普页、错误提示和无障碍文本均使用稳定 ID / `TextRef`，由 `src/i18n/catalogs.ts` 的三套 catalog 渲染。标题页主标题在简体中文和英文界面显示“记忆的缝隙”，繁体中文界面显示“記憶的縫隙”；浏览器标题、全屏入口眉题和其他界面文案仍按 catalog 渲染。Tiled 的中文 `label` 只保留给编辑器；运行时按实体稳定 ID 查找翻译。语言变化只更新 DOM 和 Canvas 实体文字，不重建章节、不改变玩法或进度。
...
感谢公开权威资料的维护者；所列机构未参与本作制作，也不代表对本作背书。医学与敏感性专业审核完成后，仅在取得同意时补充公开致谢。
