...
1. 在普通 PC 浏览器与受支持的手机横屏浏览器中稳定运行 20–30 分钟的完整垂直切片。
...
- 对白由 `#system-layer` 中覆盖整个应用的语义按钮承接指针输入，内部对白卡只负责视觉呈现；仅完整的主按钮 `click` 分派现有 `ADVANCE_DIALOGUE`，移动超过 12 CSS px、按住超过 600 ms、右键或取消手势均不推进。该层位于 HUD 之上、触控暂停之下；粗指针对白态隐藏方向、留意、情境交互及背包 / 日记 / 地图工具栏，只保留暂停，方向遮罩和暂停菜单继续拥有更高优先级。不新增领域状态或输入动作。
- 粗指针横屏显示固定四向、静静留意、情境交互和暂停控制，点击目标不小于 44×44 CSS px，并使用 `viewport-fit=cover`、动态视口单位和 `safe-area-inset-*`。情境交互键只从现有只读状态派生“交互 / 查看 / 拾取 / 放置 / 前往 / 继续 / 牵手”标签，仍只发送统一的 `interact` InputAction，不自行判断玩法结果。标题主页与子页的移动横屏布局从 740 CSS px 宽开始启用，以适配移动 Edge 工具栏下的 754×361 全屏及 754×269 非全屏视口；标题子页采用模式左右分栏、记忆三列卡和设置双列分区，返回入口统一固定在标题区右上；正常文字大小下页面面板不得滚动，放大文字只允许设置分区内部滚动。最低完整游玩支持视口仍为 780×360。
...
- `AppShell` 不渲染启动语言门；`navigator.languages` 的系统语言解析为三种 Locale，设置和暂停页的 `localePreference` 可手动覆盖并立即重渲染 DOM、Canvas 实体标签、文档元数据和字体栈。标题页主标题在简体中文和英文界面显示“记忆的缝隙”，繁体中文界面使用 catalog 中的“記憶的縫隙”；浏览器标题、全屏入口眉题、内容提示、菜单和无障碍描述仍从 catalog 按界面语言渲染。开发 DEBUG 面板不纳入翻译范围。
...
领域进展按签名变化自动保存；离开类触发额外保存当前位置，但不增加周期性或逐帧写入。成功自动保存保持静默，仅在写入失败时显示状态反馈。读取记忆本身不视为新保存。无障碍与音量设置独立写入全局设置键，不改变片段保存时间。
...
9. 尾声长按与科普页；
10. 1024×576、1366×768、1920×1080 与手机横屏 780×360 截图；
11. 移动端首次全屏进入门、拒绝/不支持降级、标题设置与暂停切换，以及新游戏、继续和读档不重复申请权限。
