...
- `AppShell` 不渲染启动语言门；`navigator.languages` 的系统语言解析为三种 Locale，设置和暂停页的 `localePreference` 可手动覆盖并立即重渲染 DOM、Canvas 实体标签、文档元数据和字体栈。只有标题页主标题固定显示简体中文“记忆的缝隙”；浏览器标题、全屏入口眉题、内容提示、菜单和无障碍描述仍从 catalog 按界面语言渲染。开发 DEBUG 面板不纳入翻译范围。
