...
每个槽存储 `SaveSlotRecordV1`，包含容器 `formatVersion`、ISO 8601 `savedAt` 和清理后的 `GameState`。当前槽只保存在运行时：开始新游戏或继续存档时先激活槽位，之后自动与手动保存均写入该槽。旧 `erasure.save.v1` 不迁移也不展示，确认“清除全部本地数据”时删除。
...
自动存档只在领域进度签名变化时更新时间戳；读取存档本身不视为新保存。无障碍与音量设置独立写入全局设置键，不改变槽位保存时间。
...
- 标题页展示三个固定槽位的章节、模式、游玩时长与保存时间；空槽按编号自动分配；
- 满槽覆盖、单槽删除及“清除全部本地数据”必须二次确认；后者同时删除三个槽、旧单槽键与设置并回到标题；
...
- 三槽 SaveData 的隔离、摘要、瞬态清理、损坏回退和存储异常；
...
5. D3 标准与低扰动两条路线；
6. 暂停、失焦与恢复；
7. 三槽自动分配、刷新恢复、手动保存、满槽覆盖和单槽删除；
8. 损坏槽隔离、全局设置保留和窄视口键盘操作；
9. 尾声长按与科普页；
...
- `canvas[data-scene-ready="true"]` 只在 Phaser 连续完成两个 `POST_RENDER` 后设置，浏览器测试据此开始截图或键盘输入，避免把 DOM 菜单就绪或首帧调度误当成 Canvas 可见帧就绪；
