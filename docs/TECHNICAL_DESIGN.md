...
非 home 地图的 visual_props 对象大多使用 placeholder（无 gid/tileset），仅携带 entityId/sortY/size 元数据。rain_station 的红伞实体使用 `prop_red_umbrella_closed` tileset；Shared Life 的照片、空相册、三件生活物件和三处放置槽使用 v02 `prop_life_shared_life_atlas` tileset。`entity.life.exit` 的正式走廊已经绘入 v02 背景，地图中只保留逻辑 `interactables` hotspot，不再创建无图像 visual_props。
