...
- 游戏内 DOM 面板在受支持横屏中使用固定面板外壳：背包与日记只滚动内容区，地图按主体与图例分栏，暂停将快捷设置与四路混音分栏并使用紧凑数据操作底栏；关闭、继续和返回标题入口始终留在视口内。面板打开期间仍由 GameState 模态状态阻断玩法输入，不把物件、日记、地图或设置状态复制到 DOM 本地状态。
